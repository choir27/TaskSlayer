  ...
  try {
    ...
    if (playlistData.$id === playlistSongs.id) {
      ...
      localStorage.setItem(
        "playlist",
        JSON.stringify({ id: playlistData.$id, song: playlistSongs.song }),
      );
    }
...
export default function AddToPlaylist(props: Music) {
  const playlist = useStore((state: State) => state.listOfSongs);
  const song = useStore((state: State) => state.song);

  const playlistOptions = playlist.map((playlist: ListOfSongs, i: number) => (
    <option key={playlist.$id} value={i}>
      {playlist.playlistName}
    </option>
  ));

  const [currentPlaylist, setCurrentPlaylist] = useState<ListOfSongs>({
    $id: "",
    playlistName: "",
    playlistSongs: [""],
    userID: "",
  ...
  return (
    <section>
      <select
        onChange={(e: any) =>
          setCurrentPlaylist({
            $id: playlist[e.target.value]?.$id,
            playlistName: playlist[e.target.value]?.playlistName,
            playlistSongs: playlist[e.target.value]?.playlistSongs,
            userID: playlist[e.target.value]?.userID,
            user: playlist[e.target.value]?.user,
          })
        }
      >
