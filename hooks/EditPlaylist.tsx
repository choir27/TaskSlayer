...
export default function EditPlaylist(props: Edit) {
  ...
  const [playlistNameInputDisplay, setPlaylistNameInputDisplay] =
    useState<boolean>(false);
  const [playlistName, setPlaylistName] = useState<string>("");

  const findPlaylist = playlist.find(
    (listOfSongs: ListOfSongs) => listOfSongs.$id === props.id,
  ) as ListOfSongs;
  ...
  return (
    <div id="tableContainer">
      ...
      <h3>
        Updated At: {date.getMonth() + 1}/{date.getDate()}/{date.getFullYear()}
      </h3>
      <table>
        <thead>
          <tr>
